...
| 网络监控 | 仅支持 Windows 10/11。启动后会非阻塞地预检 WFP 引擎并请求管理员授权；用户明确启用后，通过独立辅助进程按应用统计 TCP/UDP 流量，并保存最多 7 天的本地 SQLite 历史。 |
...
- Manager 默认关闭。应用启动后会非阻塞预检 WFP 引擎与管理员授权，但不会开始采集或创建 `network-usage.sqlite3`；用户选择开始后才累计和写入历史。
- 主应用始终保持普通权限。预检和启用监控时，Windows 通过 UAC `runas` 启动同一可执行文件的隐藏辅助模式；需要管理员权限的采集只存在于辅助进程。
