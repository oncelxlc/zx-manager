...
| 网络监控 | 仅支持 Windows 10/11。默认会在应用启动时非阻塞地授权并开始监控；关闭启动偏好后，应用启动不会请求 UAC。首次开始监控会创建本次应用运行期间复用的提权辅助进程，按应用统计 TCP/UDP 流量并保存最多 7 天的本地 SQLite 历史。 |
...
- Tauri 启动时先显示无脚本的 `public/splashscreen.html`，主窗口保持隐藏；主窗口仍先渲染 React 启动兜底，再读取偏好、应用主题、初始化 i18n 并挂载初始路由，最后通过受限命令完成窗口交接。网络监控恢复不参与交接等待条件。
...
- 默认会在应用启动后非阻塞预检 WFP 引擎与管理员授权，预检成功后开始采集并按需创建 `network-usage.sqlite3`。可在“设置 → 网络监控”关闭“应用启动时开始监控”；关闭后后续启动不会请求 UAC，该开关不会停止当前会话。
- 主应用始终保持普通权限。自动或手动首次开始监控时，Windows 通过 UAC `runas` 启动同一可执行文件的隐藏辅助模式；需要管理员权限的采集只存在于辅助进程。
- 主进程与辅助进程通过仅限本机的命名管道通信，并校验当前用户/SYSTEM ACL、启动 PID、随机 nonce 和协议版本。禁用监控会停止并丢弃 ETW runtime，但保留空闲辅助进程到应用退出；本次应用运行内再次启用不会重复请求 UAC。主进程退出、管道断开或协议失败会结束辅助进程，之后再次开始需要重新授权。
- 仅“当前会话启停”决定是否累计流量；授权预检失败不会阻止应用壳打开。采样间隔位于应用流量卡片，可选 1、3、5 或 10 秒，默认 5 秒。
...
UAC 取消、WFP 预检失败、ETW 启动失败、辅助进程断开、协议不兼容和事件丢失都会以结构化状态显示；启动时恢复失败不会阻止应用主界面打开。
