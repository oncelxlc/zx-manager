const zhCN = {
  ...
  networkMonitor: {
    title: "网络监控",
    description: "查看 Windows 上每个应用的实时与历史网络流量。",
    ...
    onboarding: {
      ...
      storageDescription: "启用时 Windows 会请求管理员授权以启动独立 ETW 辅助进程。仅保存最多 7 天的应用名、路径哈希和流量桶；不会保存完整路径、PID、远程地址、域名、URL、端口、数据包内容或命令行。",
      start: "开始监控",
      notNow: "暂不开始",
    },
    metrics: {
    ...
    quality: {
      ...
      partialDescription: "ETW 丢失事件 {{lost}} 条，无法解析应用 {{unresolved}} 条；受影响的数据不会被伪装成完整结果。",
      unknownTitle: "存在未分类网络路径",
      unknownDescription: "PAC、自动代理、透明代理、TUN 或其他无法可靠判断的连接仅计入“全部”，不会计入“代理”或“非代理”。",
      aggregateTitle: "应用聚合值说明",
      aggregateDescription: "这些数值来自应用 TCP/UDP 事件，不等同于网卡线速总量。本地代理可能同时产生客户端到代理、代理到远端两段应用流量。",
      includesUnknown: "含未分类",
    },
    realtime: {
      title: "最近 10 分钟",
      description: "图表按上方网络路径筛选应用流量；丢失或不完整的采样显示为断点。",
    },
    ...
    applications: {
      title: "应用流量",
      description: "同一可执行文件的多个进程合并为一个应用；路径筛选后的应用按当前总速率降序排列。",
      system: "系统",
      unknown: "未知应用",
      columns: {
        name: "应用",
    ...
    history: {
      title: "历史用量",
      description: "仅按时间范围和网络路径查询，结果在整个范围内按应用聚合并按总流量降序。",
      range: "时间范围",
    ...
    errors: {
      ...
      codes: {
        ...
        collectorUnavailable: "Windows ETW 应用流量采集启动失败。",
